...
.PARAMETER Repository
    The GitHub repository (owner/repo format). Default: dotnet/diagnostics
...
param(
    ...
    [string]$Repository = "dotnet/diagnostics",
...
function Test-RepositoryFormat {
    ...
    if ($Repo -notmatch $repoPattern) {
        throw "Invalid repository format '$Repo'. Expected 'owner/repo' (e.g., 'dotnet/diagnostics')."
    }
    return $true
}
