---
name: ci-analysis
description: Analyze CI build and test status from Azure DevOps and Helix for dotnet/diagnostics repository PRs. Use when checking CI status, investigating failures, determining if a PR is ready to merge, or given URLs containing dev.azure.com or helix.dot.net. Also use when asked "why is CI red", "test failures", "retry CI", "rerun tests", "is CI green", "build failed", "checks failing", or "flaky tests".
---

# Azure DevOps and Helix CI Analysis

Analyze CI build status and test failures in Azure DevOps and Helix for the dotnet/diagnostics repository and other dotnet repositories.
...
```powershell
# Analyze PR failures (most common) - defaults to dotnet/diagnostics
...
| `-Repository` | Target repo (default: dotnet/diagnostics) |
