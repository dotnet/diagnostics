...
.DESCRIPTION
    ...
    dirs.sln file to the new .slnx format, then cleans up the old file.
...
try {
    ...
    # Step 2: Check if dirs.sln exists before migration
    if (Test-Path "dirs.sln") {
        Write-Step "Migrating dirs.sln to new format..."
        try {
            & $dotnetExe sln dirs.sln migrate
            $migrateExit = $LASTEXITCODE
            if ($migrateExit -ne 0) { throw "dotnet sln migrate exited with $migrateExit" }
            Write-Success "Migration completed successfully"
        }
        catch {
            Write-ErrorMessage "Failed to migrate dirs.sln"
            Write-Host "Error details: $($_.Exception.Message)" -ForegroundColor Red
            Write-Warning "Continuing with cleanup..."
        }

        # Step 3: Clean up old solution file
        Write-Step "Cleaning up old dirs.sln file..."
        try {
            Remove-Item "dirs.sln" -Force
            Write-Success "Old solution file removed"
        }
        catch {
            Write-Warning "Could not remove dirs.sln: $($_.Exception.Message)"
            Write-Host "You may need to remove it manually" -ForegroundColor Yellow
        }
    }
    else {
        Write-Warning "dirs.sln not found, skipping migration step"
    }

    Write-Host ""
    Write-Success "Solution generation and migration completed successfully!"
    Write-Host "You can now open the generated .slnx files in Visual Studio." -ForegroundColor Gray
}
catch {
