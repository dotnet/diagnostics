    ...
    public class FindPointersInCommand : CommandBase
    {
        ...
        private RegionPointers ProcessOneRegion(bool pinnedOnly, IEnumerable<(ulong Pointer, DescribedRegion Range)> pointersFound, MemoryWalkContext ctx)
        {
            ...
            foreach ((ulong Pointer, DescribedRegion Range) found in pointersFound)
            {
                ...
                {
                    result.AddRegionPointer(found.Range, found.Pointer, isImage: true);
                }
                else
                {
                    result.AddRegionPointer(found.Range, found.Pointer, isImage: false);
                }
            }
        ...
        private class RegionPointers
        {
            ...
            internal void AddRegionPointer(DescribedRegion range, ulong pointer, bool isImage)
            {
                var pointerMap = isImage ? ResolvablePointers : UnresolvablePointers;
