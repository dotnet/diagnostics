...
namespace Microsoft.Diagnostics.DebugServices.UnitTests
{
    public class PathUtilitiesTests
    {
        ...
        public void IsSafeAbsoluteLocalPathAcceptsOnlyHostLocalAbsolutePaths()
        {
            ...
            {
                string localPath = Path.Combine(Environment.SystemDirectory, "mscordaccore.dll");
                Assert.True(PathUtilities.IsSafeAbsoluteLocalPath(localPath));
                Assert.True(PathUtilities.IsSafeAbsoluteLocalPath(localPath.Replace('\\', '/')));
                Assert.True(PathUtilities.IsSafeAbsoluteLocalPath(@"Z:\mounted\mscordaccore.dll"));
        ...
        [InlineData("Z:coreclr.dll", "coreclr.dll")]
        [InlineData("Z:Z:coreclr.dll", "coreclr.dll")]
        [InlineData(@"remote\Z:module.pdb", "module.pdb")]
        ...
        [Fact]
        public void DataReaderStripsRemotePathsBeforePassingThemToClrMd()
        {
            Assert.Equal(
                "coreclr.dll",
                DataReader.GetFilePathForClrMd(@"\\remote.example\share\coreclr.dll"));
            Assert.Equal(
                "module.pdb",
                DataReader.GetFilePathForClrMd("//remote.example/share/module.pdb"));
            Assert.Equal(
                "coreclr.dll",
                DataReader.GetFilePathForClrMd("Z:coreclr.dll"));
            Assert.Equal(
                "clr.dll",
                DataReader.GetFilePathForClrMd("Z:clr.dll"));
            Assert.Equal(
                "module.pdb",
                DataReader.GetFilePathForClrMd("Z:module.pdb"));
            Assert.Equal(
                "coreclr.dll",
                DataReader.GetFilePathForClrMd("Z:Z:coreclr.dll"));
            Assert.Equal(
                "clr.dll",
                DataReader.GetFilePathForClrMd("Z:Z:clr.dll"));
            Assert.Equal(
                "module.pdb",
                DataReader.GetFilePathForClrMd("Z:Z:module.pdb"));
        }

        [Fact]
        public void RuntimeRejectsRemoteLibraryAndRuntimePaths()
        {
        ...
        [Fact]
        public void RuntimeUsesOnlyLeafNameFromRemoteLibraryPath()
        {
            string runtimeModulePath = RuntimeInformation.IsOSPlatform(OSPlatform.Windows)
                ? @"C:\dotnet\coreclr.dll"
                : "/usr/share/dotnet/libcoreclr.so";
            ...
            Assert.Equal(Path.Combine(Path.GetDirectoryName(runtimeModulePath), "mscordaccore.dll"), candidate);
        }
